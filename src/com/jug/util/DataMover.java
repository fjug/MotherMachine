...
public class DataMover {
     ...
     * I cases where the native pixel types match, <code>convertAndCopy</code> will simply call <code>copy</code>.
     *
     * <b>General assumptions and rules:</b>
     * <li> User takes care that <code>source</code> is defines on all interval locations of <code>target</code>.
     *      (The source <i>exists</i> on all places the target has to be filled.)
     *
     * <b>Supported conversions are:</b>
     * <li> From <code>DoubleType</code>...
     * 		<li> ...to <code>ARGBType</code>; source range is expected to be subset or equal to <code>[0,1]</code>.
     * 		     If values <0 or >1 are found the source image will be scanned for maximal and minimal values and the
     * 		     converted image will be normalized, filling the entire target range <code>[0,255]</code> in all 3 color
     *               channels.
     * <li> From <code>ARGBType</code>...
     * 		<li> ...to <code>DoubleType</code>; conversion ignores A, and computed double value like this:
     *               <code>v = ( 0.2989R + 0.5870G + 0.1140B ) / 255</code>, which leads to a target
     *               <code>Img</code> that lies within <code>[0,1]</code>.
     * @param source
     * @param target
     * @throws Exception
     */
...
//    }
	...
	if ( sourceType instanceof DoubleType ) {
		...
		while (targetCursor.hasNext()) {
		    ...
		    if (v > 255) {
			throw new Exception("TODO: in this case (source in not within [0,1]) I did not finish the code!!! Now would likely be a good time... ;)");
		    }
